...
| `CCW_REQUIRE_TASK_SECTIONS`         | empty                                                       | up to 16 comma-separated headings          | Literal headings a caller-authored task body must contain, matched case-insensitively. A task missing any of them is refused before a job is created: a structural input contract, not a quality judgement. Worker-generated SDD review tasks are exempt. |
